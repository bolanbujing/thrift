...
public:
  ...
  virtual uint32_t writeFieldBegin(const char* name,
                                        ...
                                        const int16_t fieldId) {return 0;}

  virtual uint32_t writeFieldEnd() {return 0;}

  virtual uint32_t writeFieldStop() {return 0;}

  virtual uint32_t writeMapBegin(const TType keyType, const TType valType, const uint32_t size){return 0;}

  virtual uint32_t writeMapEnd() {return 0;}

  virtual uint32_t writeListBegin(const TType elemType, const uint32_t size) {return 0;}

  virtual uint32_t writeListEnd() {return 0;}

  virtual uint32_t writeSetBegin(const TType elemType, const uint32_t size) {return 0;}

  virtual uint32_t writeSetEnd() {return 0;}

  virtual uint32_t writeBool(const bool value) {return 0;}

  virtual uint32_t writeByte(const int8_t byte) {return 0;}

  virtual uint32_t writeI16(const int16_t i16) {return 0;}

  virtual uint32_t writeI32(const int32_t i32) {return 0;}

  virtual uint32_t writeI64(const int64_t i64) {return 0;}

  virtual uint32_t writeDouble(const double dub) {return 0;}

  virtual uint32_t writeString(const std::string& str) {return 0;}

  virtual uint32_t writeBinary(const std::string& str) {return 0;}
  ...
  virtual uint32_t readFieldBegin(std::string& name, TType& fieldType, int16_t& fieldId) {return 0;}

  virtual uint32_t readFieldEnd() {return 0;}

  virtual uint32_t readMapBegin(TType& keyType, TType& valType, uint32_t& size) {return 0;}

  virtual uint32_t readMapEnd() {return 0;}

  virtual uint32_t readListBegin(TType& elemType, uint32_t& size) {return 0;}

  virtual uint32_t readListEnd() {return 0;}

  virtual uint32_t readSetBegin(TType& elemType, uint32_t& size) {return 0;}

  virtual uint32_t readSetEnd() {return 0;}

  virtual uint32_t readBool(bool& value) {return 0;}

  virtual uint32_t readBool(std::vector<bool>::reference value) {return 0;}

  virtual uint32_t readByte(int8_t& byte) {return 0;}

  virtual uint32_t readI16(int16_t& i16) {return 0;}

  virtual uint32_t readI32(int32_t& i32) {return 0;}

  virtual uint32_t readI64(int64_t& i64) {return 0;}

  virtual uint32_t readDouble(double& dub) {return 0;}

  virtual uint32_t readString(std::string& str) {return 0;}

  virtual uint32_t readBinary(std::string& str) {return 0;}

  uint32_t skip(TType type){
    return ::apache::thrift::protocol::skip(*this, type);
  }

  inline boost::shared_ptr<apache::thrift::transport::TMemoryBuffer> getTransport() { return ptrans_; }

  // TODO: remove these two calls, they are for backwards
  // compatibility
  inline boost::shared_ptr<apache::thrift::transport::TMemoryBuffer> getInputTransport() { return ptrans_; }
  inline boost::shared_ptr<apache::thrift::transport::TMemoryBuffer> getOutputTransport() { return ptrans_; }
  ...
  void decrementRecursionDepth() { --input_recursion_depth_; }
...
protected:
  TProtocol(boost::shared_ptr<apache::thrift::transport::TMemoryBuffer> ptrans)
    : ptrans_(ptrans), input_recursion_depth_(0), output_recursion_depth_(0), recursion_limit_(DEFAULT_RECURSION_LIMIT)
  {}

  boost::shared_ptr<apache::thrift::transport::TMemoryBuffer> ptrans_;
