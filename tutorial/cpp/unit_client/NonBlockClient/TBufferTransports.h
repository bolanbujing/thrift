...
public:
  ...
  void consume(uint32_t len) {
    if (TDB_LIKELY(static_cast<ptrdiff_t>(len) <= rBound_ - rBase_)) {
      ...
      std::cout << "consume did not follow a borrow." << std::endl;
    }
  }
...
public:
  ...
  TMemoryBuffer(uint8_t* buf, uint32_t sz, MemoryPolicy policy = OBSERVE) {
    if (buf == NULL && sz != 0) {
      ...
      std::cout << "TMemoryBuffer given null buffer with non-zero size." << std::endl;
    }
    ...
    default:
      ...
      std::cout << "Invalid MemoryPolicy for TMemoryBuffer" << std::endl;
    }
  }
